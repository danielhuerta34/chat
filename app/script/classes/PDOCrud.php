...
Class PDOCrud {
    ...
    public $maxSize = 20000000000000000000000000000; // Max size of file allowed for file upload
    ...
    private function initializeBtnActions() {
        if ($this->settings["actionbtn"]) {
            if ($this->settings["viewbtn"]) {
                $this->enqueueBtnActions("pdocrud-actions pdocrud-button-view btn btn-info btn-sm", "javascript:;", "view", "<i class=\"fa fa-search\"></i>", "", array("title" => $this->langData["view"]));
            }
            if ($this->settings["editbtn"]) {
                $this->enqueueBtnActions("pdocrud-actions pdocrud-button-edit btn btn-warning btn-sm", "javascript:;", "edit", "<i class=\"fa fa-pencil-square-o\"></i>", "", array("title" => $this->langData["edit"]));
            }
            if ($this->settings["inlineEditbtn"]) {
                $this->enqueueBtnActions("pdocrud-actions pdocrud-button-inline_edit btn btn-warning btn-sm", "javascript:;", "inline_edit", "<i class=\"fa fa-pencil\"></i>", "", array("title" => $this->langData["inline_edit"]));
            }
            if ($this->settings["delbtn"]) {
                $this->enqueueBtnActions("pdocrud-actions pdocrud-button-delete btn btn-danger btn-sm", "javascript:;", "delete", "<i class=\"fa fa-times fa-fw\"></i>", "", array("title" => $this->langData["delete"]));
            }
            if (isset($this->settings["clonebtn"]) && $this->settings["clonebtn"]) {
                $title = isset($this->langData["clone"]) ? $this->langData["clone"] : "sffd";
                $this->enqueueBtnActions("pdocrud-actions pdocrud-button-clone btn btn-warning btn-sm", "javascript:;", "clone", "<i class=\"fa fa-clone\"></i>", "", array("title" => $title));
            }
        }
    }
     ...
     * @return   object                                  Object of class
     */
    public function formSendEmail($from, $to, $subject, $message, $saveDb = true) {
     ...
     * @return   object                                            Object of class
     */
    public function FormSteps(array $fields, $stepName, $stepType = "tabs", $attr = array()) {
    ...
    * @return   object                                         Object of class
    ...
    private function initializeHTMLContent() {
        //$this->HTMLContent[] = $this->getAjaxLoaderImage($this->settings["script_url"] . "crm/themes/panel/assets/img/store.gif");
         $this->HTMLContent[] = $this->getAjaxLoaderImage("/dirup/themes/panel/assets/img/store.gif");
        return $this;
    }
...
//        unset($this->btnActions["edit"]);
    ...
    private function getColSum($data) {
        ...
        foreach ($this->colSumTotals as $cols) {
            $colsum[] = "FORMAT(SUM(" . $cols . "), 0, 'de_DE') as " . $cols;
        }
    ...
    public function getSubmitField($fieldName, $attr = array(), $data = array(), $fieldClass = array()) {
        $class = implode(" ", $fieldClass);
        $formId = $this->formId;
        $field = "<input data-form-id=\"$formId\" type=\"submit\" class=\"btn btn-success btn-sm pdocrud-form-control pdocrud-submit $class\" id=\"$fieldName\" name=\"$fieldName\" ";
        if (is_array($attr) && count($attr)) {
            foreach ($attr as $c => $v) {
                $field .= " $c=\"$v\" ";
            }
    ...
    public function getButtonField($fieldName, $attr, $data = array(), $fieldClass = array()) {
        $class = implode(" ", $fieldClass);
        $formId = $this->formId;
        $field = "<button data-form-id=\"$formId\" type=\"button\" class=\"btn btn-danger btn-sm pdocrud-form-control pdocrud-button $class\" id=\"$fieldName\" name=\"$fieldName\" ";
        $buttonText = "";
    ...
    public function getImageField($imageSrc, $attr = array(), $imageClass = array()) {
        ...
        $image = "<div class=\"show-image\"><img src=\"$imageSrc\" class=\"$class\" ";
        if (is_array($attr) && count($attr)) {
            foreach ($attr as $c => $v) {
                $image .= " $c=\"$v\" ";
            }
    ...
    public function getLableField($lableText, $lableFor, $lableClass = array()) {
        ...
        $lable = "<label for=\"$lableFor\" class=\"$class $lableFor\">$lableText</label>";
        return $lable;
    }
    ...
    public function getAjaxLoaderImage($imagepath) {
        $ajaximg = "<div id=\"pdocrud-ajax-loader\"><img width=\"200\" src=\"$imagepath\" class=\"pdocrud-img-ajax-loader\"/></div>";
        return $ajaximg;
    }
    ...
    public function getSearchBox($columns, $data = array()) {
        ...
        $search .= "<a href=\"javascript:;\" id=\"pdocrud_search_btn\" name=\"pdocrud_search_btn\" class=\"pdocrud-form-control pdocrud-actions btn btn-primary btn-sm $class\" data-action=\"search\">";
     ...
     */
    function fileUpload($fileName, $fileUploadPath = "", $maxSize = 20000000000000000000000000000, $allowedFileTypes = array()) {
        if ($this->checkValidFileUpload($fileName, $fileUploadPath, $maxSize, $allowedFileTypes)) {
            ...
            $destinationFileName = $fileName["name"];
