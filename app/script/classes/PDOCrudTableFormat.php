...
Class PDOCrudTableFormat extends PDOCrudHelper {
    ...
    public function formatTableColOptions($col, $options, $val, $data = array()) {
        switch (strtolower($options["formatType"])) {
            case "image":
                return $this->formatTableColImage($val, $options["paramaters"]);
            case "images":
                return $this->formatTableColImages($val, $options["paramaters"]);
            case "string":
            ...
            case "rateit":
                return $this->formatTableColRateit($val, $options["paramaters"]);    
            case "expression":
                return $this->formatTableColExpression($val, $options["paramaters"], $data);        
            default:
                return $val;
        }
    }
    ...
    private function formatTableColImages($val, $parameters = "") {
        return $this->getImagesCol($val, $parameters);
    }
    ...
    private function formatTableColRateit($val, $parameters = "") {
        return "<div class='rateit' data-rateit-value='".$val."' data-rateit-ispreset='true' data-rateit-readonly='true'></div>";
    }

    private function formatTableColExpression($val, $parameters = "", $data) {
        if(count($data)){
            preg_match_all('/{(.*?)}/', $val, $matches);
            foreach ($matches[1] as $key => $value) {
               $val = str_replace("{".$value."}", $data[$value], $val);
            }
            $val = $this->parseExpression($val);
            return $val;
        }
    ...
    private function formatTableColString($val, $parameters = "") {
        switch (strtolower($parameters["type"])) {
            ...
            case "upperfirst":
                return ucfirst($val);
            case "prefix":
                if (isset($parameters["duplicate"]) && $parameters["duplicate"] === true && $this->startsWith($val, $parameters["str"]))
                    return $val;
                else
                    return $parameters["str"] . $val;
            case "suffix":
                if (isset($parameters["duplicate"]) && $parameters["duplicate"] === true && $this->endsWith($val, $parameters["str"]))
                    return $val;
                else
    ...
    private function formatTableColReadMore($val, $parameters = "") {
        if (isset($parameters["length"]) && strlen($val) > $parameters["length"]) {
            $readmoretext = "<button class='btn btn-info btn-sm'>Leer más <i class='fa fa-arrow-right'></i></button>";
    ...
    private function getImageCol($paths, $attr = "") {
        $imgField = "<img src=\"$paths\" ";
        foreach ($attr as $c => $v) {
            $imgField .= " $c=\"$v\" ";
        }
        $imgField .= " />";
    ...
    private function getImagesCol($paths, $attr = "") {
        $imgField ="";
        if(strpos($paths,",")){
            $path = explode(",", $paths);
            foreach($path as $pt){
                $imgField .= "<a class=\"fancybox\" data-fancybox=\"images\" href=\"$pt\"><img src=\"$pt\" ";
                foreach ($attr as $c => $v) {
                    $imgField .= " $c=\"$v\" ";
                }
                $imgField .= " /></a>";
