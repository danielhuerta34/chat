    ...
    jQuery.pdocrud_actions = {
        init: function () {
                ...
                jQuery(document).on('change', '.pdocrud-checkbox:checkbox', function () {
                   if (jQuery(this).is('[required]') || jQuery(this).is('[apply-req-validation]')) {
                       if (jQuery(this).is(':checked')) {
                           jQuery(this).closest(".pdocrud-checkbox-group").find(".pdocrud-checkbox").removeAttr('required');
                           jQuery(this).closest(".pdocrud-checkbox-group").find(".pdocrud-checkbox").attr('apply-req-validation',"1");
                       }
                       else {
                           if (jQuery(this).closest(".pdocrud-checkbox-group").find(".pdocrud-checkbox").is(":checked")) {
                               jQuery(this).closest(".pdocrud-checkbox-group").find(".pdocrud-checkbox").removeAttr('required');
                           }else{
                               jQuery(this).closest(".pdocrud-checkbox-group").find(".pdocrud-checkbox").attr('required', true);
                           }
                       }
                   }
               });
          ...
          if(pdocrud_js.hasOwnProperty('ajax_actions')){
            jQuery.each(pdocrud_js.ajax_actions, function (index) {
              jQuery(document).on(pdocrud_js.ajax_actions[index].event, '.'+pdocrud_js.ajax_actions[index].class, function (evt) {
                  var instance = jQuery.pdocrud_actions.getInstance(this, "form");
                  var data = {};
                  ...
                  if(pdocrud_js.ajax_actions[index].other_elements.length){
                    ...
                    jQuery.each(pdocrud_js.ajax_actions[index].other_elements, function (loop) {
                      data.post_data.other_element_name[loop] = pdocrud_js.ajax_actions[index].other_elements[loop];
                      data.post_data.other_element_value[loop] = jQuery('.pdocrud_ajax_action_other_'+pdocrud_js.ajax_actions[index].other_elements[loop]).val();
                    });                    
                  }
          ...
          if(pdocrud_js.hasOwnProperty('invoice_headers')){
            ...
            jQuery(document).on("click", "table.pdocrud-excel-table tbody tr td.pdocrud-row-cols", function (evt) {
                ...
                if(jQuery(this).find('input').is(':focus')) return this;
                var width = jQuery(this).width();
                jQuery(this).html('<input type="text" class="pdocrud-excel-cell" value="' + content + '" style="width:'+width+'px" />')
                    .find('input')
                    .trigger('focus')
                    .on({
                      'focusout': function(){
                        jQuery(this).trigger('closeCellData');
                      },
                      'keyup':function(e){
                        if(e.which == '13'){ 
                          jQuery(this).trigger('saveCellData');
                          jQuery(this).trigger('closeCellData');
                        } else if(e.which == '27'){ 
                          jQuery(this).trigger('closeCellData');
                        }
            ...
            if (jQuery(".pdocrud-spinner").length > 0) {
                    jQuery(".pdocrud-spinner").spinner({
                    step: jQuery(".pdocrud-spinner").data("step"),
                    min: jQuery(".pdocrud-spinner").data("min"),
                    max: jQuery(".pdocrud-spinner").data("max"),
                    start: jQuery(".pdocrud-spinner").data("start"),
                });
            }

            if(jQuery(".pdocrud_search_input").length > 0 && pdocrud_js.hasOwnProperty('enable_search_on_enter')) {
              jQuery(document).on("keypress",".pdocrud_search_input", function (e) {
                 if (e.which == 13 && pdocrud_js.enable_search_on_enter) {
                  jQuery("#pdocrud_search_btn").trigger("click");
                 }
              });
            }
            
            jQuery.pdocrud_actions.getAutoSuggestionData(this);
            
            jQuery.pdocrud_actions.setBulkCrudData(this);
            ...
            jQuery(document).on("change", ".pdocrud-filter", function (evt) {
                ...
                if (filters.find("span[data-key='" + key + "']").length > 0){
                    if(val)
                        ...
                        jQuery(".pdocrud-filters-options").find("span[data-key='" + key + "']").remove();
                    }
                }
                else{
           ...
           jQuery(document).on("click", ".pdocrud_add_file", function (evt) {
                evt.preventDefault();
                jQuery(this).siblings(".pdocrud-filecontrol-div").find('.pdocrud-file').click();
           });
           
           jQuery(document).on("click", ".pdocrud_remove_file", function (evt) {
                evt.preventDefault();
                jQuery(this).siblings(".pdocrud-file-input-control").val('');
           });
           
            jQuery(document).on("change", ".pdocrud-file", function (evt) {
                evt.preventDefault();
                var filePath = jQuery(this).val()
                if (filePath.match(/fakepath/)) {
                    filePath = filePath.replace(/C:\\fakepath\\/i, '');
                }
                jQuery(this).parent().siblings(".pdocrud-file-input-control").val(filePath);
            });
            ...
            jQuery(document).on('click', '.pdocrud-close', function (evt) {
                evt.preventDefault();
                jQuery(this).closest("table.pdocrud-table-view").remove();
            });
            
            jQuery(document).on('click', '.pdocrud-data-row', function (evt) {
                if (pdocrud_js.quick_view) {
                    evt.preventDefault();
                    jQuery(this).closest(".pdocrud-table").find(".pdocrud-data-row").removeClass("pdocrud-row-selected");
                    jQuery(this).addClass("pdocrud-row-selected");
                    var data = jQuery(this).data();
            ...
            jQuery(document).on('click', 'a.pdocrud-actions', function (evt) {
                ...
                if (data.action === "read_more") {
                    ...
                    if (jQuery(this).data("hide") === "true") {
                        jQuery(this).html("<button class='btn btn-info btn-sm'>Leer más <i class='fa fa-arrow-right'></i></button>");
                        jQuery(this).data("hide", "false");
                        jQuery(this).prev("p").html(content.substr(0, jQuery(this).data("length")) + "...");
                    }
                    else {
                        jQuery(this).html("<button class='btn btn-primary btn-sm'>Ocultar <i class='fa fa-eye-slash'></i></button>");
            ...
            jQuery(document).on('submit', 'form', function (evt) {
                if (pdocrud_js.submission_type === "ajax") {
                    ...
                    jQuery(this).find("input[readonly='true']").each(function () {
        ...
        setBulkCrudData : function(obj){
            if (jQuery(".input-bulk-crud-update").length > 0) {
                 jQuery(".input-bulk-crud-update").each(function(){
                     if(jQuery(this).get(0).tagName  === "SELECT")
                        jQuery(this).val(jQuery(this).data("orignal-val"));
                 });
            }
        },
        getAutoSuggestionData : function(obj){
             if (jQuery(".pdocrud_search_input").length > 0 && pdocrud_js.auto_suggestion) {
                var instance = jQuery(".pdocrud-table-container").data("objkey");
                var data = jQuery(obj).data();
                data.action = "autosuggest";
                jQuery(".pdocrud_search_input").autocomplete({
                    source: function (request, response) {
                        data.search_text = request.term;
                        data.search_col = jQuery(".pdocrud_search_cols").val();
                        jQuery.ajax({
        ...
        getRenderData :function (obj, instance, data) {
            jQuery.ajax({
                ...
                success: function (response) {
                    ...
                    jQuery(obj).closest("form").next(".pdocrud-adv-search-result").html(response);
                }
            });
        },
        actions: function (obj, data, instance, printwindow) {
                ...
                success: function (response) {
                    ...
                    if (data.action === "exporttable") {
                        ...
                        if (jQuery(obj).closest(".pdocrud-table-container").data("modal")) {
                            ...
                            var element = jQuery(obj).closest(".pdocrudbox")
                            ...
                            jQuery('html, body').animate({ scrollTop: $(".pdocrud-quick-view").last().offset().top }, 'slow');
                        }
         ...
         ajax_actions: function (obj, instance, data, return_value_element) {
            ...
            jQuery.ajax({
                ...
                success: function (response) {                    
                    jQuery(document).trigger("pdocrud_after_custom_ajax_action", [obj, response]);
                    if(return_value_element){
                      jQuery('.pdocrud_ajax_action_return_'+return_value_element).val(response);
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    console.log(textStatus, errorThrown);
